...
class Pad(mindspore.nn.Cell):
    ...
    def construct(self, input):
        ...
        new_padding = tuple(tuple(elem) for elem in padding)
        return ops.Pad(new_padding)(input)
...
class ModuleDict(mindspore.nn.Cell):
    def __init__(self, *args, **kwargs):
        ...
        super(ModuleDict, self).__init__(auto_prefix)
        if len(args) == 1:
            self.update(args[0])
...
class MultiheadAttention(mindspore.nn.MultiheadAttention):
    """
    Please input mindspore.nn.MultiHeadAttention args:
...
class Transformer(mindspore.nn.Transformer):
...
class RealTransformerEncoder(mindspore.nn.TransformerEncoder):
