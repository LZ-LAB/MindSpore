...
def vstack(tensors, *, out=None):
    ...
    return out_adaptor(mindspore.ops.concat(tuple(tensors), axis=0), out)
...
def dstack(tensors, *, out=None):
    result = mindspore.numpy.dstack(tensors)
    return out_adaptor(result, out)
...
def swapaxes(input, axis0, axis1):
    return mindspore.numpy.swapaxes(input, axis1=axis0, axis2=axis1)
...
def meshgrid(*tensors, indexing=None):
    if indexing is None:
        indexing = "ij"
    if len(tensors) == 1:
        return mindspore.numpy.meshgrid(*tuple(tensors[0]), indexing=indexing)
    else:
        return mindspore.numpy.meshgrid(*tensors, indexing=indexing)
...
def histogram(input, bins, *, range=None, weight=None, density=False, out=None):
    result = mindspore.numpy.histogram(input, bins=bins, range=range, weights=weight, density=density)
    return out_adaptor(result, out)


def histogramdd(input, bins, *, range=None, weight=None, density=False, out=None):
    result = mindspore.numpy.histogramdd(input, bins=bins, range=range, weights=weight, density=density)
    return out_adaptor(result, out)


def hypot(input, other, *, out=None):
    result = mindspore.numpy.hypot(input, other)
    return out_adaptor(result, out)


def inner(input, other, *, out=None):
    result = mindspore.numpy.inner(input, other)
    return out_adaptor(result, out)


def kron(input, other, *, out=None):
    result = mindspore.numpy.kron(input, other)
    return out_adaptor(result, out)


def lcm(input, other, *, out=None):
    result = mindspore.numpy.lcm(input, other)
    return out_adaptor(result, out)


def logaddexp(input, other, *, out=None):
    result = mindspore.numpy.logaddexp(input, other)
    return out_adaptor(result, out)


def logaddexp2(input, other, *, out=None):
    result = mindspore.numpy.logaddexp2(input, other)
    return out_adaptor(result, out)


def nanmean(input, dim=None, keepdim=False, *, dtype=None, out=None):
    result = mindspore.numpy.nanmean(input, axis=dim, dtype=dtype, keepdims=keepdim)
    return out_adaptor(result, out)
