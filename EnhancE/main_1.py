...
def save_model(model, opt, measure, args, measure_by_arity=None, test_by_arity=False, itr=0, test_or_valid='test',
               is_best_model=False):
    """
...
def main(args):
    ...
    opt = mindspore.nn.Adagrad(model.trainable_params(), learning_rate=args.lr, weight_decay=5e-6, accum=0,
                               update_slots=False)

    for name, param in x2ms_adapter.named_parameters(model):
        print('Parameter %s: %s, require_grad = %s' % (
        name, str(x2ms_adapter.tensor_api.x2ms_size(param)), str(param.requires_grad)))
    ...
    for it in range(model.cur_itr.data, args.num_iterations + 1):
        ...
        if (it % 2 == 0 and it != 0) or (it == args.num_iterations):
            print("validation:")
            ...
            if is_best_model:
                ...
                best_model.best_mrr.set_data(mindspore.Tensor(np.array([mrr])), slice_shape=False)
                best_model.best_itr.set_data(mindspore.Tensor(np.array([it])), slice_shape=False)
