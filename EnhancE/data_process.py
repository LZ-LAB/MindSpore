...
class Dataset:
    ...
    def next_batch(self, batch_size, neg_ratio, arity, device):
        ...
        # print(batch)
        # batch = mindspore.tensor(batch).long()
        batch = mindspore.tensor(batch).long()
        ...
        ms = mindspore.tensor(ms).float()
        bs = mindspore.tensor(bs).float()
        return batch, ms, bs
    ...
    def generate_neg(self, pos_batch, neg_ratio, arity):
        ...
        neg_batch = np.concatenate(
            [self.neg_each(np.repeat([c], neg_ratio * arities[i] + 1, axis=0), arities[i], neg_ratio) for i, c in
