...
class Data:
    ...
    def load_strfacts(self, data_dir='../../../data/JF17K/'):
        dataset = x2ms_adapter.tensor_api.split(data_dir.strip(), '/')[-2]
        ...
        if dataset in ['JF17K', 'FB-AUTO', 'WikiPeople1', 'WN18RR', 'FB15K-237', 'WikiPeople-3', 'WikiPeople-4', 'JF17K-3', 'JF17K-4']:
            for filename in ['train', 'valid', 'test']:
                ...
                for k in range(len(lines)):
                    line = x2ms_adapter.tensor_api.split(lines[k].strip(), '\t')
            ...
            for filename in ['train', 'valid', 'test']:
                ...
                for k in range(len(lines)):
                    ...
                    if dic['N'] > max_ary:
                        max_ary = dic['N']
                    tmp_fact = []
                    str_list = [x.strip('":') for x in x2ms_adapter.tensor_api.split(line.strip('\n{}'))]
                    role_list = []
