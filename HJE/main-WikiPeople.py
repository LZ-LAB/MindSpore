...
import argparse
import time
device = x2ms_adapter.Device('cuda:2' if x2ms_adapter.is_cuda_available() else 'cpu')
...
class Experiment:
    ...
    def train_and_eval(self):
        ...
        if self.dr:
            scheduler = mindspore.nn.ExponentialDecayLR(learning_rate=self.lr, decay_rate=self.lr,
        ...
        for it in range(1, self.num_iterations + 1):
            ...
            for ary in args.ary_list:
                ...
                for miss_ent_domain in range(1, ary+1):
                    ...
                    def construct(j):
                        j = j
                        data_batch, label, rel_idx, ent_idx = self.get_batch(er_vocab, er_vocab_pairs, j,
                                                                             miss_ent_domain)

                        # ents_idx = data_batch[:, 1:]
                        def forward_fn(rel_idx, ent_idx, miss_ent_domain, label):
                            pred = model.construct(rel_idx, ent_idx, miss_ent_domain)
                            loss = model.loss(pred, label)
                            return loss, pred

                        grad_fn = mindspore.value_and_grad(forward_fn, None, opt.parameters, has_aux=True)
                        (loss, _), grads = grad_fn(rel_idx, ent_idx, miss_ent_domain, label)
                        opt(grads)
                        return loss
                ...
                if self.dr:
                    lr = scheduler(it)
                    opt = mindspore.nn.Adam(model.trainable_params(), learning_rate=lr)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="WikiPeople", nargs="?", help="FB-AUTO/JF17K/WikiPeople/WN18RR/FB15K-237.")
    ...
    parser.add_argument("--lr", type=float, default=0.00024, nargs="?", help="Learning rate.")
    # parser.add_argument("--dr", type=float, default=0.995, nargs="?", help="Decay rate.")
    parser.add_argument("--dr", type=float, default=None, nargs="?", help="Decay rate.")
