...
class Experiment:
    ...
    def get_batch(self, er_vocab, er_vocab_pairs, idx, miss_ent_domain):
        ...
        targets = x2ms_adapter.zeros((len(batch), len(d.ent2id)), device=None)
        for idx, pair in enumerate(batch):
            targets[idx, er_vocab[pair]] = 1.
        batch = mindspore.tensor(batch, dtype=mindspore.int64)
        r_idx = batch[:, 0]
    ...
    def get_test_batch(self, test_data_idxs, idx, miss_ent_domain):
        batch = mindspore.tensor(test_data_idxs[idx:idx+self.batch_size], dtype=mindspore.int64)
        r_idx = batch[:, 0]
        e_idx = batch[:, [i for i in range(1, batch.shape[1]) if i != miss_ent_domain]]
    ...
    def train_and_eval(self):
        ...
        model = HJE(len(d.ent2id), len(d.rel2id), self.dembd, self.dembd1, self.max_ary)
        opt = mindspore.nn.Adam(model.trainable_params(), learning_rate=self.lr)
        if self.dr:
